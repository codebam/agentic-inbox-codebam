 ...
 *
 * This is a pure module shared by the browser (search hooks / result views)
 * and the server (the aggregated /api/v1/search route, the agent and MCP
 * tools), so a raw query string can be passed straight through and parsed in
 * exactly one place.
 *
 * Supported operators:
...
/**
 * Build the API query params for the per-mailbox search route from a raw
 * Gmail-style query. The aggregated /api/v1/search route takes the raw `q`
 * instead and parses it server-side; this helper exists for the older
 * per-mailbox route, which expects already-parsed filters.
 */
